...
import type { MachineTrendPoint } from '@/services/productionService'

export default function OeeTrendChart({ data, target }: { data: MachineTrendPoint[]; target?: number }) {
    ...
    <div className="h-72 w-full">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={chartData} margin={{ top: 10, right: 10 }}>
          ...
          {target !== undefined && <ReferenceLine y={target} stroke="#8a97a8" strokeDasharray="4 4" />}
          <Line type="monotone" dataKey="oee" stroke="#1e4480" strokeWidth={2.5} dot={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  )
